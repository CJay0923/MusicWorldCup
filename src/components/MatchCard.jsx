 ...
 */
export default function MatchCard({ entrant, side, state, showSideTag, onPick, onPreview }) {
  const classes = [
    'card',
    side,
    state,
    entrant?.isSeed ? 'seed-card' : '',
  ].filter(Boolean).join(' ');
  ...
  return (
    <div className={classes} onClick={onPick}>
      ...
      <div className={`album-wrap${entrant?.pic ? '' : 'empty'}`}>
        {entrant?.pic && (
      ...
      <div className="check" style={{ opacity: state === 'win' ? 1 : 0 }}>✓</div>
    </div>
  );
}
