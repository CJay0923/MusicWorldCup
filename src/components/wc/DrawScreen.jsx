  ...
  return (
    <div className={clsx('wc-overlay', show && 'wc-overlay--show')}>
      <div className="wc-panel">
        <h2 className="wc-panel-title">⚽ 抽签结果</h2>
        <p className="wc-sub">
        ...
        <div className="wc-draw-grid">
          {groups.map((g) => (
            <div key={g.name} className="wc-draw-group">
              <div className="wc-draw-group-name">
                <span>{g.name}</span>
                <span className="wc-draw-group-suffix">组</span>
              </div>
              {g.members.map((idx, mi) => {
                const e = entrants[idx];
                const sr = seedRank[idx] || 999;
                return (
                  <div key={mi} className="wc-draw-song">
                    <span
                      className="wc-seed-badge"
                      style={{ visibility: sr <= 32 ? 'visible' : 'hidden' }}
                    >
                      {sr <= 32 ? `#${sr}` : ''}
                    </span>
                    <span>{e.name}</span>
                    <span className="wc-pool-pos">P{mi + 1}</span>
                  </div>
                );
              })}
            </div>
